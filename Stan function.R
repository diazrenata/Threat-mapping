...
runStanModel <- function(threat, predictors, states, ncores, CAR, scramble){
  ...
  predictorDf$Richness <- rowSums(predictorDf@data[,SppCols])
  ...
  workPath <- paste0('H:/Global Change Program/Research/Multi-Threat Assessment/Analysis - Threat Mapping/CAR beginning 07132017/CountyEndangeredSpecies-master/CAR Analyses/', threat, "_", predictorString, "_st", states, '_scr', scramble, '_CAR', CAR)
  ...
  FitModel = sampling(StanModel,
                      ...
                      iter = 2500,                 # total number of iterations per chain (includes warm-up)
  ...
  resultsShape@data[AllIncludedFeatures$featureShape,"geo_effect"] = summary(FitModel, pars = paste("geo_effect[",1:length(AllIncludedFeatures$featureShape),"]", sep=""))$c_summary[,,1][,"50%"]
  ...
  if (CAR == FALSE) resultsShape@data <- resultsShape@data[ , c('FIPS_CODE_LONG', 'X', 'STATE_NAME', 'STATEFP', 'Richness', 'detections', 'FeatureID', 'state_coeff', 'p_est','p_calc_justX', 'p_calc_notGeo' ,'geo_effect')]

  #   palette <- brewer.pal(9, 'YlOrBr')
  #   palette <- c(palette, '#000000')
  #   palette <-  cbind(palette, c(0:9))
  #   colnames(palette) <- c('Colour', 'pColour')
  #   palette <- as.data.frame(palette)
  #
  #   palette_p_est <- palette
  #  # palette_p_calc<- palette
  #   palette_p_calc_notGeo <- palette
  #   palette_p_calc_justX <- palette
  #
  #   colnames(palette_p_est) <- c('p_estColour', 'p_estVal')
  #  # colnames(palette_p_calc) <- c('p_calcColour', 'p_calcVal')
  #   colnames(palette_p_calc_notGeo) <- c('p_calc_notGeoColour', 'p_calc_notGeoVal')
  #   colnames(palette_p_calc_justX) <- c('p_calc_justXColour', 'p_calc_justXVal')
  #
  #   resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_estVal"] <- floor(as.numeric(resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_est"] /0.1))
  #   #resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_calcVal"] <- floor(as.numeric(resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_calc"]) / 0.1)
  #   resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_calc_notGeoVal"]  <- floor(as.numeric(resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_calc_notGeo"]) / 0.1)
  #   resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_calc_justXVal"] <- floor(as.numeric(resultsShape@data[AdMatrixData$geoInfo$sampledId$featureShape,"p_calc_justX"]) / 0.1)
  #
  #
  #   resultsShape@data <- join(resultsShape@data, palette_p_est, by = 'p_estVal', type = 'left')
  #   #resultsShape@data <- join(resultsShape@data, palette_p_calc, by = 'p_calcVal', type = 'left')
  #   resultsShape@data <- join(resultsShape@data, palette_p_calc_notGeo, by = 'p_calc_notGeoVal', type = 'left')
  #   resultsShape@data <- join(resultsShape@data, palette_p_calc_justX, by = 'p_calc_justXVal', type = 'left')
  #
  setwd(workPath)
  ...
  if (CAR == TRUE) {
    tiff(paste0(threat, "_", predictorString, "_", 'p_calc.tiff'), width = 8000, height = 5000, units = "px")
  ...
  tiff(paste0(threat, "_", predictorString, "_", 'p_calc_notGeo.tiff'), width = 8000, height = 5000, units = "px")
  ...
  tiff(paste0(threat, "_", predictorString, "_", 'p_calc_justX.tiff'),width = 8000, height = 5000, units = "px")
  ...
  if (CAR == TRUE){
    ...
    tiff(paste0(threat, "_", predictorString, "_", 'alpha_tau.tiff'), width = 1000, height = 1000, units = "px")
  ...
  tiff(paste0(threat, "_", predictorString, "_", 'likelihood.tiff'), width = 1000, height = 1000, units = "px")
  ...
  tiff(paste0(threat, "_", predictorString, "_", 'logloss.tiff'), width = 1000, height = 1000, units = "px")
  ...
  #Does the model-estimated Ps perform better than random?
  if ( CAR == TRUE ) summ5 <- summary(FitModel, pars = c("logloss_random", "logloss_obs", "logloss_calc_p", "logloss_notGeo", "logloss_justX" ),  use_cache = F)$summary
  if (CAR == FALSE) summ5 <- summary(FitModel, pars = c("logloss_random", "logloss_obs", "logloss_notGeo", "logloss_justX" ),  use_cache = F)$summary
  ...
  #Calls r squareds
  if (CAR == TRUE)  summ2<- summary(FitModel, pars = c("r_sq","r_sq_notGeo","r_sq_justX"))$summary
  if (CAR==FALSE) summ2<- summary(FitModel, pars = c("r_sq_notGeo","r_sq_justX"))$summary

  if (CAR == TRUE) summ3 <- summary(FitModel, pars = c("r_sq_log","r_sq_notGeo_log","r_sq_justX_log"))$summary
  if (CAR == FALSE)  summ3 <- summary(FitModel, pars = c("r_sq_notGeo_log","r_sq_justX_log"))$summary
  ...
  if ( CAR == FALSE) {
    ...
    rsquared.justX.mean <- summ2[1,1]
    rsquared.justX.2pt5 <- summ2[1,4]
    rsquared.justX.97pt5 <- summ2[1,8]
  ...
  write.csv(results, 'H:/Global Change Program/Research/Multi-Threat Assessment/Analysis - Threat Mapping/CAR beginning 07132017/CountyEndangeredSpecies-master/results.csv')
