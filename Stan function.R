...
runStanModel <- function(threat, predictors, states, ncores, CAR, scramble, justEndemics, islands){
  ...
  natureServeCounties <- natureServeCounties[ which(natureServeCounties$STATE_NAME != 'District of Columbia'), ]
  ...
  if (CAR == FALSE){
   # setwd('E:/ecology/Drive/Research/Threat mapping/countyEndangered_NotEndemics_notCAR.stan')
  ...
  tiff(paste0('p_est.tiff'), width = 5000, height = 3000, units = "px")
  ...
  if (CAR == TRUE) {
    tiff(paste0('pclc.tiff'), width = 8000, height = 5000, units = "px")
  ...
  tiff(paste0('pclc_nG.tiff'), width = 8000, height = 5000, units = "px")
  ...
  tiff(paste0('pc_jX.tiff'),width = 8000, height = 5000, units = "px")
  ...
  tiff(paste0('X.tiff'),width = 8000, height = 5000, units = "px")
  ...
  # save plots of descriptive parameters (rsq, alpha_tau, ps, likelihood, logloss)
  tiff('rsq.tiff', width = 1000, height = 1000, units = "px")
  ...
  if (CAR == TRUE){
    ...
    tiff(paste0('a_t.tiff'), width = 1000, height = 1000, units = "px")
    print(stan_dens(FitModel,pars = c("alpha","tau"),separate_chains = T )+  labs(title = paste0(threat, '_', predictorString, " alpha_tau")))
    dev.off()
    
  }
  
  tiff(paste0('ps.tiff'), width = 1000, height = 1000, units = "px")
  print(stan_dens(FitModel,pars = c("p[1]","p[50]"),separate_chains = T )+  labs(title = paste0(threat, '_', predictorString, " ps")))
  dev.off()
  
  tiff(paste0('lklhd.tiff'), width = 1000, height = 1000, units = "px")
  print(stan_plot(FitModel, pars = c("random_lh", "obs_lh"), show_density=T, ci_level=0.95, outer_level=.99)+  labs(title = paste0(threat, '_', predictorString, " likelihood")))
  dev.off()
  
  tiff(paste0('lglss.tiff'), width = 1000, height = 1000, units = "px")
  ...
  if (CAR == FALSE)  print(stan_plot(FitModel, pars = c("logloss_random", "logloss_obs", "logloss_notGeo", "logloss_justX" ), show_density=T, ci_level=0.95, outer_level=.99)+ geom_vline(xintercept=expectedRandomLogLoss, linetype="dashed", color = "red")+  labs(title = paste0(threat, '_', predictorString, " logloss")))
  ...
  # save states effects
  if (states == T) {
    stateeffects <- stan_plot(FitModel, pars = paste("a_cat[",1:nHyperP,"]", sep=""), show_density=T, ci_level=0.95, outer_level=0.99)+ geom_vline(xintercept=0, linetype="dashed", color = "red")
    ...
    tiff(paste0( 'steff.tiff'), width = 1000, height = 1000, units = "px")
    print(
      stateeffects
    )
    dev.off()
